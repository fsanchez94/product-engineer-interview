    ...
    def handle(self, *args, **options):
        ...
        for name, commission in category_data:
            category, created = Category.objects.get_or_create(
                name=name, defaults={"commission_override": commission}
            )
        ...
        for name, cat_name, price, inventory, seller_idx in products_data:
            ...
            product = Product.objects.create(
                ...
                cost=Decimal(
                    str(price * random.uniform(0.4, 0.7))
        ...
        for order in orders:
            num_items = random.choices([1, 2, 3, 4], weights=[40, 35, 20, 5], k=1)[
                0
            ...
            for _ in range(num_items):
                ...
                quantity = random.choices([1, 2, 3], weights=[70, 25, 5], k=1)[
                    0
                ]  # Most single quantity
                ...
                if random.random() < 0.2:
                    promo_discount_amount = base_price * Decimal(
                        "0.10"
                    )  # 10% promo discount from original price
                ...
                total_discount_percent = (
                    (total_discount / base_price * 100) if base_price > 0 else 0
                )
            ...
            shipping = (
                Decimal("9.99") if order_total < 50 else Decimal("0")
                    ...
                    if random.random() < 0.4:
                        Review.objects.create(
                            ...
                            rating=random.choices(
                                [1, 2, 3, 4, 5], weights=[2, 3, 10, 35, 50], k=1
                            )[
                                0
                            ],  # Mostly positive
            ...
            AnalyticsEvent.objects.create(
                ...
                metadata={
                    ...
                    "user_agent": random.choice(
                        ["Chrome/91.0", "Safari/14.1", "Firefox/89.0"]
                    ),
                },
        ...
        self.stdout.write(
            self.style.SUCCESS("Database seeded with comprehensive analytics data!")
        )
        self.stdout.write(
            self.style.SUCCESS(
                f"Created {len(sellers)} sellers, {len(products)} products, {len(orders)} orders"
            )
        )
