...
from marketplace.models import AnalyticsEvent, OrderItem
...
def get_seller_sales_performance(seller_id):
    ...
    product_stats = (
        order_items.values("product__product_id", "product__name")
        .annotate(revenue=Sum("price_at_purchase"), quantity_sold=Sum("quantity"))
        .order_by("-revenue")
    )
...
def get_seller_market_share(seller_id):
    ...
    seller_revenue = (
        OrderItem.objects.filter(
            product__seller__seller_id=seller_id,
            order__status__in=["paid", "shipped", "delivered"],
        ).aggregate(total=Sum("price_at_purchase"))["total"]
        or 0
    )
    ...
    for item in seller_category_revenue:
        ...
        category_market_share.append(
            {"category": category_name, "share_percentage": round(share_percentage, 2)}
        )
...
def get_platform_category_market_share():
    ...
    for item in category_revenue:
        ...
        percentage = (
            (revenue / float(total_platform_revenue)) * 100
...
def get_platform_top_products():
    ...
    top_products = (
        OrderItem.objects.filter(order__status__in=["paid", "shipped", "delivered"])
        .values("product__product_id", "product__name", "product__category__name")
        .annotate(
            total_revenue=Sum("price_at_purchase"),
    ...
    return {"top_products": products_data}
...
def get_platform_search_analytics():
    ...
    product_searches = (
        AnalyticsEvent.objects.filter(event_type="search", product__isnull=False)
        .select_related("product", "product__category")
        .values("product__product_id", "product__name", "product__category__name")
        .annotate(search_count=Count("id"))
        .order_by("-search_count")[:20]
    )  # Top 20 most searched products
        ...
        search_data.append(
            {
                ...
                "percentage": round((item["search_count"] / total_searches) * 100, 2)
                if total_searches > 0
                else 0,
            }
        )

    return {"total_searches": total_searches, "most_searched_products": search_data}
...
def get_platform_revenue_by_state():
    ...
    orders = Order.objects.filter(status__in=["paid", "shipped", "delivered"]).values(
    ...
    for order in orders:
        ...
        if shipping_addr:
            ...
            state = (
                ...
                or shipping_addr.get("State")
                or shipping_addr.get("province")
                or shipping_addr.get("Province")
                or "Unknown"
            )
        else:
        ...
        if state in state_revenue:
            ...
            state_revenue[state] = {"revenue": float(order["total"]), "orders": 1}

    # Convert to list format and sort by revenue
    states_data = []
    for state, data in state_revenue.items():
        percentage = (
            (data["revenue"] / total_platform_revenue) * 100
            if total_platform_revenue > 0
            else 0
        )
        states_data.append(
    ...
    return {"total_platform_revenue": total_platform_revenue, "states": top_states}
